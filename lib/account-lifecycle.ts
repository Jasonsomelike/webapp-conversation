...
export const deleteAppUserAccountByUsername = async ({
  username,
  actorUserId,
  allowSelf = false,
}: {
  ...
  const users = await withDatabaseRetry(() => db.$queryRaw<Array<{ id: string }>>`
    SELECT "id"::text AS "id"
    FROM "app_users"
...
export const cleanupHiddenAdminUsers = async ({
  actorUserId,
}: {
  actorUserId: string
}) => {
  ...
  await ensureAccountLifecycleStorage()
  const users = await withDatabaseRetry(() => db.$queryRaw<Array<{ id: string, username: string }>>`
    SELECT "id"::text AS "id", "username"
    FROM "app_users"
    WHERE "deleted_at" IS NULL
      AND (
        LOWER("username") LIKE 'guest_%'
        OR LOWER("username") LIKE 'test%'
        OR LOWER("username") LIKE 'demo%'
        OR LOWER("username") LIKE 'deleted_%'
      )
  ...
  for (const user of users) {
    results.push({
      ...
      result: await deleteAppUserAccount({
        appUserId: user.id,
        actorUserId,
        allowSelf: false,
      }),
  ...
  return { ok: true, deleted: results.length, users: results.map(item => item.username) }
}
